...
CRITICAL - Rank-appropriate content:
- 1SG/MSG: Senior enlisted leaders. They've COMPLETED ALC and SLC. Reference SMC, USASMA, or Master's programs - NOT ALC.
- SFC: Completed ALC, attending or completed SLC. Reference SLC, not ALC.
- SSG: Attending or completed ALC. Reference ALC achievements.
- SGT: Completed BLC, may reference ALC selection.
...
NCO BACKGROUND:
- Current rank: ${ctx.rank || "1SG"}
- Current position: ${ctx.position || "First Sergeant"}
- Previous position: ${ctx.previousPosition || ""}
- Completed PME: ${ctx.completedPME ? ctx.completedPME.join(", ") : "BLC, ALC, SLC/MLC (already completed)"}
- ${ctx.recommendedForSMC ? "RECOMMENDED FOR SMC/USASMA in previous eval" : ""}
- ${ctx.education ? `Education: ${ctx.education}` : ""}
- ${ctx.broadeningAssignment ? `Current role is a BROADENING ASSIGNMENT` : ""}

CRITICAL: Do NOT reference ALC/SLC completion - this 1SG has already completed those. Focus on:
- SMC selection/preparation
- Graduate education progress
- Senior-level professional development
- Cross-functional expertise (paralegal background in 1SG role)

${ctx.previousIntellect ? `Previous comments for writing style:\n${ctx.previousIntellect}\n` : ""}

Write bullet-style comments about mental agility, sound judgment, and professional development appropriate for a senior enlisted leader.`,
    maxTokens: 200,
  },
...
IMPORTANT: The rated NCO is currently ${ctx.rank || "1SG"} (use this rank, not any previous rank).
${ctx.previousOverall ? `Previous comments for reference style only:\n${ctx.previousOverall}\n` : ""}
...
IMPORTANT: The rated NCO is currently ${ctx.rank || "1SG"} (use this rank, not any previous rank).
${ctx.previousRater ? `Previous assessment for reference style only:\n${ctx.previousRater}\n` : ""}
...
IMPORTANT: The rated NCO is currently ${ctx.rank || "1SG"} (use this rank, not any previous rank).
${ctx.previousSR ? `Previous evaluation for reference style only:\n${ctx.previousSR}\n` : ""}
...
IMPORTANT: The rated NCO is currently ${ctx.rank || "1SG"} McShane (use this rank, not any previous rank like MSG).
${ctx.previousSRComments ? `Previous comments for reference style only:\n${ctx.previousSRComments}\n` : ""}
