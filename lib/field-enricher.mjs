...
function buildContextForField(field, previousNcoer, unitConfig) {
  ...
  if (previousNcoer?.supplementalContext) {
    // Pass ALL supplemental context to LLM for maximum info
    Object.assign(context, previousNcoer.supplementalContext);
  }

  // Also pass the previous eval metadata
  if (previousNcoer?.metadata) {
    context.previousRank = previousNcoer.metadata.rank;
    context.previousPosition = previousNcoer.metadata.principalDutyTitle;
    context.previousUnit = previousNcoer.metadata.unit;
  }
